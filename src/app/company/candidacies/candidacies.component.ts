...
export class CandidaciesComponent implements OnInit {
  ...
  public getCandidacies(){
    ...
    this.http.get("http://backend-tic-enit.herokuapp.com/offers/candidacies?id="+id ,{ headers: reqHeader }).subscribe((data : any) => {
  ...
  public getUserInfo(id : string){
    var reqHeader = new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem("companyToken")});
    this.http.get("http://backend-tic-enit.herokuapp.com/company/user/"+id ,{ headers: reqHeader }).subscribe((data : any) => {
      this.user = data;
      if(data.picture != ""){
